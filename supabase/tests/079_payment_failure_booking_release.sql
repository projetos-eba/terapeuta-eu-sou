...
create temporary table payment_failure_test_slot on commit drop as
select
  (
    current_date
    + ((3 - extract(dow from current_date)::integer + 7) % 7)
    + 14
    + time '10:10'
...
insert into public.bookings (
  ...
  'draft',
  'not_started'
from payment_failure_test_slot slot;
  ...
  (
    (
      public.reserve_booking_hold_v1(
        ...
        'd1000000-0000-4000-8000-000000000001',
        (select starts_at from payment_failure_test_slot),
        (select ends_at from payment_failure_test_slot),
        'America/Sao_Paulo',
