-- Restore the public directory's live next-slot projection after the therapy
-- category retirement transition. The card must reflect the first slot from
-- any eligible online service, using the same authoritative slot engine as
-- the reservation flow.
create or replace function public.get_public_therapist_next_slot_v1(p_therapist_profile_id uuid)
returns timestamptz
language sql
stable
security definer
...
as $$
  ...
  join public.therapies candidate_therapy
    on candidate_therapy.id = candidate.therapy_id
  ...
  where candidate.therapist_profile_id = p_therapist_profile_id
...
revoke all on function public.get_public_therapist_next_slot_v1(uuid) from public;
grant execute on function public.get_public_therapist_next_slot_v1(uuid) to anon, authenticated, service_role;
...
select therapist.id as therapist_profile_id, therapist.slug, therapist.public_name,
  ...
  public.get_public_therapist_next_slot_v1(therapist.id) as next_slot_at,
...
comment on function public.get_public_therapist_next_slot_v1(uuid) is
  'Returns the first currently bookable slot across every eligible online service for a public therapist.';
