-- The slot engine applies p_limit before excluding active bookings and holds.
-- Evaluate the 31-day horizon in five-day windows. With the canonical minimum
-- 15-minute cadence, a window has at most 484 raw candidates (including a DST
-- fallback hour), so the 500 candidate limit is applied only after every raw
-- candidate in that window has been considered.
...
left join lateral (
  ...
  where candidate.therapist_profile_id = therapist.id and candidate.status = 'active' and candidate.is_bookable and candidate.online_only and candidate_therapy.status = 'published' and candidate_therapy.is_public_visible and candidate_category.is_active and public.is_public_service_booking_eligible_v1(candidate.id)
) next_slot on true
