...
-- Keep the payout position introduced by 20260904123000, but derive the
-- blocked card from the canonical therapist receipt status. A terminal
-- refunded payment can retain transfer_status = blocked for its history; it
-- must not be presented as an active operational block.
...
begin
  ...
  v_summary := coalesce(v_payload->'summary', '{}'::jsonb)
    || jsonb_build_object(
      ...
      'payoutProcessingCents', v_processing_cents
    );
...
comment on function public.get_private_therapist_payouts_v2(
  date, date, text, integer, integer, text
) is 'Repasses v2; processamento exclui sessoes futuras no estado A receber e o card bloqueado considera apenas impedimentos operacionais ativos derivados.';
