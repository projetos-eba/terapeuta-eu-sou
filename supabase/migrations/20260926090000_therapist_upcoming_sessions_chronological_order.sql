-- Future sessions must be presented from the nearest upcoming slot onward.
-- Keep the historical list reverse-chronological and preserve the existing
-- cursor contract by deriving direction from the open-ended future window.
...
as $$
declare
  v_ascending boolean := p_period_start is not null and p_period_end is null;
...
begin
  ...
  with candidates as (
    ...
    order by
      case when v_ascending then session_row."startsAt" end asc,
      case when v_ascending then session_row."bookingId" end asc,
      case when not v_ascending then session_row."startsAt" end desc,
      case when not v_ascending then session_row."bookingId" end desc
    limit p_limit + 1
  ),
  page as (
    select *
    from candidates
    order by
      case when v_ascending then "startsAt" end asc,
      case when v_ascending then "bookingId" end asc,
      case when not v_ascending then "startsAt" end desc,
      case when not v_ascending then "bookingId" end desc
    limit p_limit
  ),
  payload as (
    select
  ...
  select
    ...
    (
      ...
      from payload
      order by
        case when v_ascending then "startsAt" end desc,
        case when v_ascending then "bookingId" end desc,
        case when not v_ascending then "startsAt" end asc,
        case when not v_ascending then "bookingId" end asc
      limit 1
    )
...
comment on function public.get_therapist_sessions_v1(
  ...
  'Therapist-scoped sessions read model. Future open-ended windows are chronological; historical windows remain reverse-chronological. Identity comes from auth.uid().';
