-- Fail closed when the T-24 worker changes payment state after the patient
-- opened the cancellation dialog. The decision is made from freshly locked
-- server state; browser state is never authoritative.
...
comment on function public.cancel_uncharged_session_v10(uuid,uuid,text,text) is
  'Cancels only a pristine V10 scheduled charge from freshly locked server state and rejects stale browser decisions after charge processing begins.';
