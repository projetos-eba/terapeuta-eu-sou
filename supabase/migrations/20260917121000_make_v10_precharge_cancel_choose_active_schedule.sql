-- The reschedule replacement and its superseded predecessor can share the
-- same transaction timestamp. Always select the active schedule before any
-- historical row so a pristine replacement is not rejected as stale.
...
begin
  ...
  -- There is at most one non-historical schedule. Prefer it explicitly rather
  -- than using only a timestamp, because reschedule transitions are atomic.
...
comment on function public.cancel_uncharged_session_v10(uuid,uuid,text,text) is
  'Cancels only a pristine V10 scheduled charge from freshly locked server state, prioritizes its active schedule, validates its version binding, and rejects stale browser decisions after charge processing begins.';
