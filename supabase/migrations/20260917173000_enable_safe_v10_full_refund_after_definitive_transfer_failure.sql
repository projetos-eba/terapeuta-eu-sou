-- A transfer job can fail before it reaches Stripe (for example during the
-- charge or connected-account preflight). That is a definitive no-transfer
-- outcome: a full customer refund is safe and must not manufacture therapist
-- debt. Jobs prepared for Stripe, leased, or otherwise ambiguous remain
-- blocked for provider reconciliation.
...
begin
  ...
  if v_job.id is not null and (
    v_job.status in ('creating', 'reconciliation_required', 'partially_reversed')
    or v_job.lease_owner is not null
    -- `failed` is safe only when the worker never prepared a provider call.
    or (v_job.status = 'failed' and (
      v_job.prepared_at is not null or v_job.stripe_transfer_id is not null
    ))
  ...
  insert into public.session_refund_decisions_v10 (
    ...
    case when v_transfer.stripe_transfer_id is not null
      and v_transfer.status <> 'reversed'
      then v_job.therapist_gross_amount_cents else 0 end,
...
begin
  ...
  if found then return jsonb_build_object('available', false, 'state',
    case when v_payment.financial_status = 'refunded' then 'refunded' else 'in_review' end);
  end if;
  select * into v_job from public.session_transfer_jobs
  where session_payment_id = p_session_payment_id;
  return jsonb_build_object('available',
    ...
    and (v_job.id is null or (
      v_job.lease_owner is null
      and (
        (v_job.status in ('queued', 'pending_source', 'transferred', 'offset_only', 'reversed')
        ...
        or (v_job.status = 'failed'
          and v_job.prepared_at is null and v_job.stripe_transfer_id is null)
      )
    )),
