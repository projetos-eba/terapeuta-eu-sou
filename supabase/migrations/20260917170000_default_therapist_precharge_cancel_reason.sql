-- The therapist cancellation dialog deliberately accepts an optional note.  The
-- booking state transition, however, always needs an audit reason.  Keep the
-- user note when supplied and use a neutral operational reason otherwise.
...
declare
  ...
  v_cancellation_reason text := coalesce(
...
comment on function public.cancel_therapist_uncharged_session_v10(uuid, uuid, text, text) is
  'Cancels only a pristine V10 scheduled charge for the assigned therapist. A blank optional note is recorded with the neutral therapist-cancellation audit reason.';
