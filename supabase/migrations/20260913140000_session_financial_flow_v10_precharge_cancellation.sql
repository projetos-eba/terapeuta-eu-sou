-- V10 phase 5: cancel only a pristine, not-yet-attempted T-24 schedule.
-- Claimed/retried/ambiguous charges remain a support operation; a concurrent
-- Stripe request must never be orphaned by an apparently successful cancel.
...
begin
  ...
  -- Payment -> booking -> schedule matches the charge-recording path. The
  -- schedule lock serializes this decision with the worker's claim.
