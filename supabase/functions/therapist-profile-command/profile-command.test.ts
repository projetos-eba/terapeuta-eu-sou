...
Deno.test(
  "validates a photo media draft without requiring profile content",
  () => {
    const result = validateTherapistProfileCommand({
...
Deno.test(
  "accepts a requested HTTPS legacy video URL for draft preservation",
  () => {
    const result = validateTherapistProfileCommand({
      action: "save_draft",
      expectedVersion: 1,
    ...
    assertEquals(result.action, "save_draft");
    if (result.action === "save_draft") {
      assertEquals(result.preserveLegacyVideoUrl, true);
    }
  },
);
