...
Deno.test("loginWithPasswordOrMaster accepts the registered password first", async () => {
...
Deno.test("loginWithPasswordOrMaster accepts the master password after password grant fails", async () => {
  const restoreFetch = stubFetch([
...
Deno.test("loginWithPasswordOrMaster accepts master password for admin role", async () => {
  const restoreFetch = stubFetch([
    textResponse(401, '{"error":"invalid_grant"}'),
    jsonResponse({
...
Deno.test("loginWithPasswordOrMaster rejects wrong master password", async () => {
...
Deno.test("loginWithPasswordOrMaster maps Supabase email_not_confirmed to a confirmation error", async () => {
  const restoreFetch = stubFetch([
    textResponse(
      400,
...
Deno.test("loginWithPasswordOrMaster does not bypass unconfirmed email with master password", async () => {
  const restoreFetch = stubFetch([textResponse(401, '{"error":"invalid"}')]);
  const client = mockClient({
    getRows: [
      [{
