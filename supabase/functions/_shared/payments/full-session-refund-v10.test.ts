...
function harness(
  ...
  const transferId =
    "transferId" in options ? (options.transferId ?? null) : "tr_original";
  ...
  const client = {
    ...
    rpc(name: string, args: unknown) {
      ...
      if (name === "transition_full_session_refund_step_v10") {
        ...
        if (transition.p_step === "reversal")
          state.reversal = transition.p_state;
  ...
  const stripe = {
    ...
    transfers: {
      ...
      listReversals: () =>
        Promise.resolve({ data: reversals, has_more: false }),
...
Deno.test(
  "full refund reverses the frozen Transfer once and refunds the complete Charge",
  async () => {
    const h = harness();
    const result = await run(h);
    assertEquals(result.status, "completed");
    assertEquals(h.counts(), { reversalPosts: 1, refundPosts: 1 });
    const reversal = h.calls.find(
      (call) => call.name === "reconcile_session_transfer_reversal_v10",
    );
    const refund = h.calls.find(
      (call) => call.name === "reconcile_session_refund_event_v10",
    );
    assertEquals(
      (reversal?.args as Record<string, unknown>).p_amount_cents,
      8500,
    );
    assertEquals(
      (refund?.args as Record<string, unknown>).p_amount_cents,
      10_000,
    );
    assertEquals(
      h.calls.filter(
        (call) => call.name === "complete_therapist_change_refund_v1",
      ).length,
      1,
    );
  },
...
Deno.test(
  "insufficient connected balance does not prevent the full customer refund",
  async () => {
...
Deno.test(
  "Stripe nested insufficient-balance errors create the compensation path",
  async () => {
    const h = harness({
      reversalError: {
        ...
        raw: {
          ...
          message:
            "Insufficient funds in your Stripe balance to complete this transfer reversal.",
        },
      },
    });
    const result = await run(h);
...
Deno.test(
  "the exact Stripe recipient-balance response creates the compensation path",
  async () => {
    const h = harness({
      reversalError: {
        type: "StripeInvalidRequestError",
...
Deno.test(
  "network errors mentioning balance without a provider 400 remain ambiguous",
  async () => {
    const h = harness({
      reversalError: new Error(
        "connection reset while checking insufficient balance",
      ),
    });
    const result = await run(h);
    assertEquals(result.status, "needs_review");
    assertEquals(h.state.reversal, "unknown");
  },
);

Deno.test(
  "a definitive no-Transfer outcome refunds the customer without a reversal or therapist debt",
  async () => {
...
Deno.test(
  "provider-side partial refund stops before any new provider mutation",
  async () => {
