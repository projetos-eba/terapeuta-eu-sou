...
export async function runFullSessionRefundV10(input: {
  ...
  const previous = await client.get<
    Array<{
      id: string;
  ...
  const decision: Decision = existing
    ? {
        ...
        reversalState: existing.reversal_state,
  ...
  if (
    ...
    charge.amount !== decision.amountCents ||
    charge.currency !== "brl" ||
  ...
  if (
    ...
    (refunds.data.length === 1 &&
      refunds.data[0].amount !== decision.amountCents)
  ) {
  ...
  if (decision.transferId) {
    ...
    if (
      transfer.source_transaction !== decision.chargeId ||
    ...
    if (reversals.has_more)
      throw new Error("full_refund_reversal_history_incomplete");
    let reversed = 0;
    ...
    if (reversed > transfer.amount)
      throw new Error("full_refund_reversal_exceeds_transfer");
    if (reversed === transfer.amount) {
      if (
        decision.reversalState === "attempting" ||
        decision.reversalState === "unknown"
      ) {
      ...
      try {
        ...
        const state = isInsufficientConnectedBalance(error)
          ? "unavailable"
          : "unknown";
  ...
  if (refund.status === "succeeded") {
    ...
    const completed =
      reconciliation.status === "reconciled" && !recoveryUncertain;
    if (completed) {
  ...
  if (
    decision.refundState === "attempting" ||
    decision.refundState === "not_attempted"
  ) {
    await transition(
      client,
      decision.decisionId,
