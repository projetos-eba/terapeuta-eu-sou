...
export async function getAuthorizedVideoBooking(input: {
  ...
  >(
    `/rest/v1/session_payments?select=financial_status,refund_pending,admin_blocked_at&booking_id=eq.${
      encodeURIComponent(
        input.bookingId,
      )
    }&limit=1`,
  );
  const pendingTherapistChanges = await input.client.get<Array<{ id: string }>>(
    `/rest/v1/booking_reschedule_requests?select=id&booking_id=eq.${
      encodeURIComponent(
        input.bookingId,
      )
    }&status=eq.pending&change_kind=in.(therapist_reschedule,therapist_cancellation)&limit=1`,
  );
  const hasPendingTherapistChange = pendingTherapistChanges.length > 0;
  ...
  >(
    `/rest/v1/video_sessions?select=id,session_name,session_key,status,provider_session_id,hard_ends_at,termination_requested_at,termination_confirmed_at,therapist_first_joined_at,therapist_last_left_at,therapist_present&booking_id=eq.${
      encodeURIComponent(
        input.bookingId,
      )
    }&limit=1`,
  ...
  if (
    !videoSession && payment?.financial_status === "paid" &&
