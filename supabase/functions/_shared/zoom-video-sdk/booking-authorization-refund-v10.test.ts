...
Deno.test("a pending support refund blocks room access for both participants", async () => {
  for (const role of ["patient", "therapist"] as const) {
      ...
      get(path: string) {
        if (path.startsWith("/rest/v1/bookings")) {
          return Promise.resolve([{
            ...
            version: 1,
            patient_profile_id: "patient_1",
            therapist_profile_id: "therapist_1",
            starts_at: "2098-09-14T13:00:00.000Z",
            ends_at: "2098-09-14T13:50:00.000Z",
    ...
    const booking = await getAuthorizedVideoBooking({
      ...
      environment: "test",
      profileId: role === "patient" ? "patient_1" : "therapist_1",
      role,
    });
    ...
    if (access.allowed) {
      throw new Error("room was available while a refund was pending");
    }
  }
});

Deno.test("a pending therapist change blocks room access before a refund review exists", async () => {
  let ensured = false;
  const client = {
    get(path: string) {
      if (path.startsWith("/rest/v1/bookings")) {
        return Promise.resolve([{
          id: "b1190000-0000-4000-8000-000000000012",
          version: 1,
          patient_profile_id: "patient_1",
          therapist_profile_id: "therapist_1",
          starts_at: "2098-09-14T13:00:00.000Z",
      ...
      if (path.startsWith("/rest/v1/booking_reschedule_requests")) {
        return Promise.resolve([{ id: "change_1" }]);
      }
      if (path.startsWith("/rest/v1/video_sessions")) return Promise.resolve([]);
      if (path.startsWith("/rest/v1/booking_events")) return Promise.resolve([]);
      return Promise.resolve([]);
  ...
  const booking = await getAuthorizedVideoBooking({
    bookingId: "b1190000-0000-4000-8000-000000000012",
    client: client as never,
    environment: "test",
    profileId: "patient_1",
    role: "patient",
  ...
  if (booking.financialStatus !== "payment_under_review" || ensured) {
    throw new Error("pending therapist change did not block room creation");
  }
  const access = evaluateVideoSessionAccess({
    actorRole: "patient",
    bookingStatus: booking.bookingStatus,
  ...
  if (access.allowed) throw new Error("room was available while a choice was pending");
});
