...
| `Modal` | P1 | Decisão crítica. | Cancelamento, confirmação, Admin. | default, danger, form. | open, loading. | `title`, `description`, `actions`. |
...
| `MatchingQuestionCard` | P0 | Capturar tema emocional. | `/sua-jornada`. | category, subcategory. | selected, disabled, maxReached. | `title`, `description`, `image`, `selected`. | Público/Paciente. |
