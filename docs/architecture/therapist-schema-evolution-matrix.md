...
| comandos `therapist_block_*_v1/v2`       | criação, remoção e resolução idempotentes; criação v2 retorna conflitos confirmados e pagos | Edge Function de Bloqueios | restrita | somente `service_role`; booking não é alterado; `session_payments` é a fonte financeira |
