...
export function TherapistJourneyThemesForm({ bookingId }: { bookingId: string }) {
  const [status, setStatus] = useState<"loading" | "available" | "saved" | "error">(
    "loading",
  );
  ...
  return (
    <section className="border-t border-brand-lavender/70 pt-6" aria-labelledby="journey-themes-title">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 id="journey-themes-title" className="text-base font-extrabold text-brand-deep">
            Temas da jornada <span className="font-semibold text-tesText-muted">(opcional)</span>
          </h3>
          <p className="mt-1 text-sm font-semibold leading-6 text-tesText-secondary">
            Registre até três temas para acompanhar esta jornada no seu Histórico.
          </p>
        </div>
      </div>
      ...
      {status === "saved" && storedSelection ? (
        <div className="mt-4 rounded-2xl border border-status-success/30 bg-status-successBg/60 p-4">
          ...
          <ul className="mt-3 flex flex-wrap gap-2" aria-label="Temas registrados">
            {storedSelection.themeKeys.map((key) => (
              <li className="rounded-full bg-white px-3 py-1.5 text-sm font-extrabold text-brand-deep" key={key}>
      ...
      {status === "available" || status === "error" ? (
        <div className="mt-4 grid gap-4">
          ...
          {errorMessage ? (
            <p aria-live="assertive" className="flex items-start gap-2 text-sm font-semibold leading-6 text-status-error">
