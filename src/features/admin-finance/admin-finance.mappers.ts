...
export function mapAdminFinanceDetail({
  ...
  return {
    backHref: module === "payments" ? routes.admin.payments : routes.admin.subscriptions,
    events: events.filter(isRecord).map((event) => mapFinanceEvent(event, record)),
    generatedAt,
    id,
    module,
...
function getDetailSections(
  ...
  if (module === "payments") {
    return [
      section("Pagamento", [
        ...
        field("Transferência", asText(record.transfer_status)),
  ...
  return [
    ...
    section("Ciclo e preço", [
      field("Ciclo atual", formatPeriod(record.current_period_start, record.current_period_end)),
