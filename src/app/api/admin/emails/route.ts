...
export async function POST(request: Request) {
  ...
  const session = await readAdminSessionFromAccessToken(config, token).catch(
    () => null,
  );
  const action =
    typeof body === "object" && body && "action" in body
      ...
      : null;
  const permission =
    action === "list" || action === "get" || action === "preview"
      ? "admin.settings.read"
      : "admin.settings.manage";
  if (!session || !canUseAdminPermission(session.permissions, permission))
    return NextResponse.json(
      { ok: false, error: { message: "Acesso administrativo necessário." } },
      { status: 403 },
    );
  const response = await fetch(
    `${config.url}/functions/v1/admin-email-management-command`,
    {
      method: "POST",
      cache: "no-store",
      headers: {
  ...
  return NextResponse.json(await response.json().catch(() => ({ ok: false })), {
