  ...
  for (const role of ["patient", "therapist", "admin"]) {
    test(`${role}: therapist absence at ${viewport.width}px`, async ({
      ...
      if (role === "patient") {
        ...
        await expect(
          page.getByText("Financeiro — independente da confirmação", { exact: true }),
        ).toBeVisible();
        await expect(page.getByText("Pago", { exact: true }).first()).toBeVisible();
        await expect(page.getByText(/avaliação de qualidade indisponível/i)).toBeVisible();
        await expect(page.getByText(/Pendente: o cliente e o terapeuta/i)).toHaveCount(0);
        await expect(
...
for (const viewport of [
  ...
  for (const role of ["patient", "therapist"]) {
    for (const stage of ["open", "overdue", "answered"]) {
      test(`${role}: quality ${stage} at ${viewport.width}px`, async ({ page, context }) => {
        await page.setViewportSize(viewport);
        await context.route("**/*", (route) => new URL(route.request().url()).origin === origin
          ? route.continue() : route.abort());
        const errors: string[] = [];
        page.on("pageerror", (error) => errors.push(error.message));
        await page.goto(`${origin}/attendance-harness?role=quality-state-${role}&stage=${stage}`);
        await expect(page.getByRole("heading", { name: stage === "answered"
          ? "Realizada (confirmada)" : "Realizada, em análise", exact: true })).toBeVisible();
        await expect(page.getByText(`Sua confirmação individual permanece pendente. Prazo automático: ${role === "patient" ? 7 : 30} dias após o horário final previsto.`, { exact: true })).toBeVisible();
        await expect(page.getByRole("link", { name: "Acompanhar meu relato no suporte" }))
          .toHaveAttribute("href", `${role === "patient" ? "/app" : "/terapeuta"}/suporte/own-private-ticket`);
        if (stage === "overdue") await expect(page.getByText(/O prazo de 5 dias do TES venceu/)).toBeVisible();
        if (stage === "open") await expect(page.getByText(/em até 5 dias corridos após o relato/)).toBeVisible();
        if (stage === "answered") await expect(page.getByText(/em até 5 dias corridos após o relato/)).toHaveCount(0);
        expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
        expect(errors).toEqual([]);
      });
    }
  }
  test(`Admin: performed quality audit at ${viewport.width}px`, async ({ page, context }) => {
    await page.setViewportSize(viewport);
    await context.route("**/*", (route) => new URL(route.request().url()).origin === origin
      ? route.continue() : route.abort());
    const errors: string[] = [];
    ...
    await expect(page.getByText("Realizada, em análise", { exact: true }).first()).toBeVisible();
    await expect(page.getByText(/Análise de qualidade: somente auditoria/)).toBeVisible();
    await expect(page.getByRole("link", { name: "Responder no ticket vinculado" }))
      .toHaveAttribute("href", "/admin/suporte/quality-ticket");
    await expect(page.getByRole("button", { name: /reembolso|reagendamento/i })).toHaveCount(0);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
    expect(errors).toEqual([]);
  });
  for (const scenario of [
    ...
    test(`${scenario.role}: quality feedback at ${viewport.width}px`, async ({ page, context }) => {
      await page.setViewportSize(viewport);
      await context.route("**/*", (route) =>
        new URL(route.request().url()).origin === origin
          ? route.continue()
          ...
          await route.fulfill({
            ...
            body: JSON.stringify({
              ...
              data: {
                ...
                actorRole: scenario.role === "quality-patient" ? "patient" : "therapist",
                status: "eligible",
                feedback: null,
              },
            }),
      ...
      await expect(page.getByText("Essa sessão foi bem-sucedida?", { exact: true })).toBeVisible();
      await page.getByRole("button", { name: scenario.successful ? "Sim" : "Não", exact: true }).click();
      if (scenario.successful) {
        await expect(page.getByRole("radiogroup", { name: "Nota de 1 a 5" })).toBeVisible();
        await page.getByRole("button", { name: "5 estrelas" }).click();
        await expect(page.getByText("Problema de internet")).toHaveCount(0);
      } else {
        await expect(page.getByRole("radiogroup", { name: "Nota de 1 a 5" })).toHaveCount(0);
        await expect(page.getByText("Sessão remarcada")).toHaveCount(0);
        await expect(page.getByText("Cancelamento em cima da hora")).toHaveCount(0);
        await page.getByLabel("Problema de internet").check();
      }
      await page.getByRole("button", { name: "Enviar feedback" }).click();
      await expect(page.getByText("Sua avaliação foi registrada")).toBeVisible();
